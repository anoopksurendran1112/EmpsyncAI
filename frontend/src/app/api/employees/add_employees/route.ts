...
export async function POST(req: Request) {
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      for (const [key, value] of formData.entries()) {
        // Handle checkbox values
        if (key === 'is_whatsapp' || key === 'is_sms' || key === 'is_wfh' || key === 'is_active') {
          formDataObj[key] = value === 'true' || value === '1' || value === 'on';
        } else if (key === 'role_id' || key === 'company_id' || key === 'group_id') {
          // Convert numeric fields to numbers if they have values
          formDataObj[key] = value && value !== '' ? Number(value) : null;
        } else if (key !== 'prof_img') { // Exclude file from object conversion
          formDataObj[key] = value;
        }
      }
      ...
      console.log("Processed body object:", body);
      console.log("Company ID:", body.company_id, "(type:", typeof body.company_id, ")");
      console.log("Role ID:", body.role_id, "(type:", typeof body.role_id, ")");
      console.log("Group ID:", body.group_id, "(type:", typeof body.group_id, ")");
      ...
      Object.entries(body).forEach(([key, value]) => {
        if (value !== null && value !== undefined && value !== '') {
          if (typeof value === 'boolean') {
            djangoFormData.append(key, value ? 'true' : 'false');
          } else {
            djangoFormData.append(key, value.toString());
          }
        }
      ...
      // Debug: Log what's being sent to Django
      console.log("=== API ROUTE: Sending to Django ===");
      console.log("Django FormData entries:");
      for (const [key, value] of djangoFormData.entries()) {
