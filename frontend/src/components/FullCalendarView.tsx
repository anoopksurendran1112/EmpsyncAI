...
import { Calendar as CalendarIcon, AlertCircle, RefreshCw, Check, X, Clock, PartyPopper, ChevronLeft, ChevronRight, Umbrella } from "lucide-react";
...
interface AttendanceEvent {
  title: string;
  start: string;
  status: 'present' | 'absent' | 'half_day' | 'holiday' | 'leave'; // Add 'leave' status
  check_in?: string;
  check_out?: string;
  hours_worked?: number;
  reason?: string;
...
const fetchAttendanceData = async (year: number, month: number) => {
  ...
  try {
    ...
    if (attendanceResult && (attendanceResult.success !== false)) {
      ...
      // Group records by date to handle duplicates
      ...
      Object.keys(recordsByDate).forEach(date => {
        ...
        const hasPresentRecord = records.some(record => 
          record.status === 'present' || 
          record.status === 'half_day'
        );
        
        // Check if there's a holiday record for this date
        const hasHolidayRecord = records.some(record => record.status === 'holiday');
        
        // Check if there's a leave record for this date
        const hasLeaveRecord = records.some(record => 
          record.type === 'leave' || 
          record.reason?.toLowerCase().includes('leave')
        );
        
        // Priority: Present/Half-day > Holiday > Leave > Absent
        let selectedRecord = records[0]; // Default to first record
        
        if (hasPresentRecord) {
          // If present, ignore leave records for the same date
          selectedRecord = records.find(record => 
            record.status === 'present' || record.status === 'half_day'
          ) || records[0];
          
          console.log(`📅 ${date}: Present/Half-day found, ignoring leave records`);
        } else if (hasHolidayRecord) {
          // If holiday, use holiday record
          selectedRecord = records.find(record => record.status === 'holiday') || records[0];
        } else if (hasLeaveRecord) {
          // If leave and no present/holiday, use leave record
          selectedRecord = records.find(record => 
            record.type === 'leave' || 
            record.reason?.toLowerCase().includes('leave')
          ) || records[0];
        ...
        // Transform the selected record to event
        let title = '';
        let status = selectedRecord.status;
        
        // Check if this is a leave
        const isLeave = selectedRecord.type === 'leave' || 
                        selectedRecord.reason?.toLowerCase().includes('leave');

        if (isLeave && !hasPresentRecord && !hasHolidayRecord) {
          // Only show leave if there's no present/half-day/holiday
          title = `LEAVE - ${selectedRecord.reason || 'Leave'}`;
          status = 'leave';
        } else if (hasPresentRecord) {
          // Present or half-day takes priority
          switch (selectedRecord.status) {
            case 'present':
              title = `PRESENT ${selectedRecord.hours_worked ? `(${selectedRecord.hours_worked}h)` : ''}`;
              break;
            case 'half_day':
              title = `HALF DAY ${selectedRecord.hours_worked ? `(${selectedRecord.hours_worked}h)` : ''}`;
              break;
            default:
              title = selectedRecord.type?.toUpperCase() || 'UNKNOWN';
          }
        } else if (hasHolidayRecord) {
          title = `HOLIDAY - ${selectedRecord.reason || ''}`;
          status = 'holiday';
        } else if (selectedRecord.status === 'absent' && !isLeave) {
          title = 'ABSENT';
        } else {
          title = selectedRecord.type?.toUpperCase() || 'UNKNOWN';
        }
        ...
        attendanceEvents.push({
          title: title,
          start: selectedRecord.date,
          status: status,
          check_in: selectedRecord.check_in,
          check_out: selectedRecord.check_out,
          hours_worked: selectedRecord.hours_worked,
          reason: selectedRecord.reason,
          type: selectedRecord.type || 'attendance',
          source: 'attendance'
        });
      });
      ...
      console.log('✅ Combined calendar events:', {
        ...
        byDate: Object.keys(recordsByDate).length
      });
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'holiday':
       ...
       case 'leave': // Add leave icon
      return <Umbrella className="h-4 w-4 text-purple-600" />; // Using Umbrella or you can import a different icon
      default:
        ...
        {eventsForDate.map((event, index) => (
          <div key={index} className="text-xs">
            <div className={`font-medium ${
              ...
              event.status === 'half_day' ? 'text-yellow-700' :
              event.status === 'leave' ? 'text-purple-700' : 'text-gray-700'
            }`}>
              {event.title.split(' - ')[0]}
            </div>
            ...
            {event.reason && (
              <div className={`pl-1 ${
                event.status === 'holiday' ? 'text-blue-600' : 
                event.status === 'leave' ? 'text-purple-600' :'text-gray-600'
              }`}>
                {event.reason}
              </div>
            )}
  ...
  const renderCalendar = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      }else if (eventsForDate.some(event => event.status === 'leave')) {
        backgroundColor = 'bg-purple-50'; // Add leave background
      } 
       else if (isToday) {
        ...
        >
          <div className="flex justify-between items-start">
            <span className={`text-sm font-medium ${
              ...
              eventsForDate.some(event => event.status === 'leave') ? 'text-purple-700' : '' // Add leave color
            }`}>
              {day}
            </span>
    ...
    <Card>
      ...
      <CardContent>
        ...
        <div className="flex flex-wrap gap-4 mb-4 p-3 bg-gray-50 rounded-lg">
          ...
          <div className="flex items-center gap-2">
            <Umbrella className="h-4 w-4 text-purple-600" /> {/* Use Umbrella or import a leave icon */}
